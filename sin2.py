...
def single_inference(speaker_prompt_audio_path, content_to_synthesize, output_path, cosyvoice, bopomofo_converter, speaker_prompt_text_transcription=None):
    ...
    speaker_prompt_text_transcription = converter.convert(
        cosyvoice.frontend.text_normalize_new(speaker_prompt_text_transcription, split=False)
    )

    content_to_synthesize = converter.convert(
        cosyvoice.frontend.text_normalize_new(content_to_synthesize, split=False)
    )
...
def main():
    ...
    #     model_source="/proj/MR_dataset/mtk53732/pretrained_models/bert-base-chinese"
    # )
    # 20260506 
    bopomofo_converter = G2PWConverter(
        model_dir = "../g2pw"
